...
from utils.metrics import find_connect
from tqdm import tqdm
import numpy as np
from mmengine.structures import PixelData
from mmdet.evaluation.functional import INSTANCE_OFFSET
from visual_scripts.pred_draw import draw_building_pred, draw_cell_pred, draw_boundary_pred, draw_cell_color
...
def train_one_epoch(model, train_loader, optimizer, logger, device, cfg, print_interval):
    ...
    for i_batch, sampled_batch in enumerate(tqdm(train_loader, ncols=70)):
        ...
        if cfg.use_boundary_head:
            ...
            targets = gt_foreground_masks.reshape(-1, h, w)
        ...
        if (i_batch+1) % print_interval == 0:
            info_str = f'iteration {i_batch+1}/{len_loader}, bce_dice_loss: {loss.item():.6f}'
...
def val_one_epoch(model, val_loader, evaluators, device, cfg, visual_args=None):
    ...
    for i_batch, sampled_batch in enumerate(tqdm(val_loader, ncols=70)):
        ...
        for idx,datainfo in enumerate(sampled_batch['data_samples']):
            ...
            pred_foreground_mask = (logits[idx,0,:,:].detach() > 0).type(torch.uint8)
            
            if cfg.use_boundary_head:
                pred_boundary_mask = logits[idx,1,:,:].detach() > 0
                pred_foreground_mask[pred_boundary_mask] = 0
                pred_inst_mask = find_connect(pred_foreground_mask, dilation=True)
                pred_foreground_mask = torch.as_tensor(pred_inst_mask > 0).type(torch.uint8)
            ...
            datainfo.pred_sem_seg = PixelData(sem_seg=pred_foreground_mask[None])
            all_datainfo.append(datainfo.to_dict())

            if 'inst' in cfg.metrics:
                gt_inst_mask = sampled_batch['gt_inst_mask'][idx]
                evaluators['inst_evaluator'].process(gt_inst_mask, pred_inst_mask)
            
            if visual_args is not None and visual_args['visual_pred'] and i_batch % visual_args['visual_pred']==0:
